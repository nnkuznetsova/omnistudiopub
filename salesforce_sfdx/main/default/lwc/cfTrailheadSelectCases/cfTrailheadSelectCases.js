import { FlexCardMixin } from "%vlocity_namespace%/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels, loadCssFromStaticResource } from "%vlocity_namespace%/flexCardUtility";
          ...
          export default class cfTrailheadSelectCases extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @api objectApiName;
              @track _omniSupportKey = 'cfTrailheadSelectCases';
        ...
        pubsub.register(`omniscript_action`,this.pubsubEvent[0]);

        this.pubsubEvent[1] = {
          data: this.handleEventAction.bind(this, data.events[1],1)
        };

        pubsub.register(`newAccountSelected`,this.pubsubEvent[1]);

              }

              unregisterEvents(){
                pubsub.unregister(`omniscript_action`,this.pubsubEvent[0]);
pubsub.unregister(`newAccountSelected`,this.pubsubEvent[1]);
